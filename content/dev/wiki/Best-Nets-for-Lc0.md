...
---
In general, for game analysis and long calculation time per move, the largest network compatible with your hardware is recommended. In use cases with low calculation time per move or slow hardware, a smaller network might be a better choice.

| Network Size | Recommended for | Filters | Blocks | Heads | GPU Memory Usage | File Size | Network |
|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Very Large | GPU | 1024 | 15 | 32 | 3.0GB | 320MB | [BT4-1024x15x32h-swa-6147500](https://storage.lczero.org/files/networks-contrib/big-transformers/BT4-1024x15x32h-swa-6147500.pb.gz) (Right-click → "Save link as...") |
| Medium | GPU/CPU | 512 | 15 | 16 | 1.6GB | 150MB | [T3-512x15x16h-distill-swa-2147500](https://storage.lczero.org/files/networks-contrib/t3-512x15x16h-distill-swa-2147500.pb.gz) (Right-click → "Save link as...") |
| Small | GPU/CPU | 256 | 10 | 8 | 1.5GB | 35MB | [T1-256x10-distilled-swa-2432500](https://storage.lczero.org/files/networks-contrib/t1-256x10-distilled-swa-2432500.pb.gz) (Right-click → "Save link as...") |
| Very Small | Sparring vs. Humans | ≤128 | ≤10 | - | - | ≤10 MB | see below |

Networks above were created using data from official Training Run1 (still active) generated by a 768x15x24 network (T80).

If you're getting `out of memory` errors when using large networks on GPU, pick the next best network in the list or try adding `--backend-opts=max_batch=256` to LC0 command (or UCI option: `BackendOptions: max_batch=256`), default: 1024. This will reduce GPU memory usage without impacting playing strength. With the cuDNN backend you can also try `--backend-opts=custom_winograd=false` or as a UCI option: `BackendOptions: custom_winograd=false`.
...
Outdated networks listed for completeness and historical purposes, some download links might be outdated.
