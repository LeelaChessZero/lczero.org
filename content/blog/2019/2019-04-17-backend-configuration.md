...
Notes:  
  ...
  2. Integer and Float are not interchangeable. If backend expects float parameter "max-temperature", it's not possible to write max-temperature=74, only max-temperature=74.0.
  3. Strings can be quoted either with " or ' marks.
  4. If string doesn't have special characters, quotation marks can be omitted, e.g. backend=cudnn-fp16 is possible instead of backend="cudnn-fp16".
  5. Keys (or subdictionary names) can not repeat, e.g. this is forbidden: backend(gpu=0),backend(gpu=1).  
 _(subnote: keys of different types are allowed [
cow=4,cow=4.0,cow(),cow=moo], but it's better not to know about that)._

  6. If subdictionary has empty list of options, parenthesis can be removed, e.g. a=5,b,c=7 instead of a=5,b(),c=7.
  7. Alternatively, if a name of a subdictionary is not needed, it can be omitted, e.g. (gpu=0),(gpu=1) instead of a(gpu=0),b(gpu=1). _(Internally, names [0], [1], etc. are created for them)._
  8. If a parameter is missing in subdictionary, it's also looked up in parent dictionary. E.g. backend=cudnn,(gpu=0),(gpu=1) is the same as (gpu=0,backend=cudnn),(gpu=1,backend=cudnn).
