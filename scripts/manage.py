        ...
        for root, dirs, files in os.walk(CONTENT_DIR):
            for file in files:
                ...
                with open(filename, "w") as f:
                    f.write(
                        "---\n%s---\n%s" % (hugo_file["header"], wiki_pages[wikiname])
                    )
...
---
%s"""
                    % (json.dumps(title), json.dumps(file), wiki_pages[file])
                )
...
@cli.command()
@click.option(
    "--quiet/--loud", default=False, help="Don't output anything when nothing happens."
)
